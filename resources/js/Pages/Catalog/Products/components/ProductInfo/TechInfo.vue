    ...
    return {
        ...
        details: {
            ...
            height: `${conversion.value === "metric" ? injectedProduct.value.dimension_height : (+injectedProduct.value.dimension_height / IMPERIAL_lENGTH).toFixed(2)} <span class='text-slate-400'>${conversion.value === "metric" ? "cm" : "inch"}</span>
             / ${conversion.value === "metric" ? +injectedProduct.value.dimension_height * CM_TO_M : (+injectedProduct.value.dimension_height * CM_TO_FOOT).toFixed(2)} <span class='text-slate-400'>${conversion.value === "metric" ? "m" : "ft"}`,
            ...
            / ${conversion.value === "metric" ? +injectedProduct.value.dimension_length * CM_TO_M : (+injectedProduct.value.dimension_length * CM_TO_FOOT).toFixed(2)} <span class='text-slate-400'>${conversion.value === "metric" ? "m" : "ft"}`,
            width: `${conversion.value === "metric" ? injectedProduct.value.dimension_width : (+injectedProduct.value.dimension_width / IMPERIAL_lENGTH).toFixed(2)} <span class='text-slate-400'>${conversion.value === "metric" ? "cm" : "inch"}</span>
              / ${conversion.value === "metric" ? +injectedProduct.value.dimension_width * CM_TO_M : (+injectedProduct.value.dimension_width * CM_TO_FOOT).toFixed(2)} <span class='text-slate-400'>${conversion.value === "metric" ? "m" : "ft"}`,
